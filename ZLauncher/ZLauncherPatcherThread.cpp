...
// Implement worker thread used in the launcher patching process
//
//////////////////////////////////////////////////////////////////////////
...
wxDEFINE_EVENT(wxEVT_COMMAND_PATCHER_UPDATE_PROGRESS_TEXT, wxThreadEvent);
...
ZLauncherPatcherThread::ZLauncherPatcherThread(ZLauncherFrame* handler, const wxString& updateURL, const wxString& versionFile, const wxString& targetDirectory)
...
ZLauncherPatcherThread::~ZLauncherPatcherThread()
{
...
wxThread::ExitCode ZLauncherPatcherThread::Entry()
{
...
//////////////////////////////////////////////////////////////////////////
bool ZLauncherPatcherThread::CheckForUpdates(const std::string& updateURL, const uint64_t& currentBuildNumber)
{
	size_t length = updateURL.find_last_of('/');

	// We don't need to download anything anymore.  We've got the XML ready to go.
	// But, rapidjson wants to see the text file. so I'm leaving this like this.
	...
	wxThreadEvent* updateProgressTextEvent = new wxThreadEvent(wxEVT_COMMAND_PATCHER_UPDATE_PROGRESS_TEXT);
	...
	// Get the relative launch path out of the xml file
	rapidxml::xml_node<>* relativeLaunchPath = zupdaterNode->first_node("relative_launch_path");
	if (!relativeLaunchPath)
	{
		ZPatcher::Log(ZPatcher::LOG_FATAL, "An error occurred while attempting to parse the XML file: Missing `relative_launch_path` node.");
		return false;
	}

	// Don't set it globally yet.  Store it with the game first.
	// Actaully this is fine...  Since the game is already selected.
	// https://stackoverflow.com/questions/875249/how-to-get-current-directory
	
	TCHAR buffer[MAX_PATH] = { 0 };
	GetModuleFileName(NULL, buffer, MAX_PATH); // C:\Users\ed\Documents\Git\ZPatcher\_Output\x64\Release\ZLauncher\ZLauncher.exe
	std::wstring::size_type pos = std::wstring(buffer).find_last_of(L"\\/");
	std::wstring tempFolderName = std::wstring(buffer).substr(0, pos);



	m_pHandler->m_LaunchExecutableName = tempFolderName + "\\" + relativeLaunchPath->value();
	//m_ApplicationName = relativeLaunchPath->value();
...
uint64_t ZLauncherPatcherThread::GetLatestVersion()
{
...
bool ZLauncherPatcherThread::GetSmallestUpdatePath(const uint64_t& sourceBuildNumber, const uint64_t& targetBuildNumber, std::vector<unsigned int>& path, uint64_t& pathFileSize)
{
...
bool ZLauncherPatcherThread::GetTargetCurrentVersion(const std::string& configFile, uint64_t& version)
{
	FILE* targetFile;

	// Each game is going to have it's own version, and version file...
	// We don't need the config file anymore...  
	// version files will be: 192587293045.version
...
std::string ZLauncherPatcherThread::MD5File(std::string fileName)
{
	FILE* targetFile;
...
bool ZLauncherPatcherThread::SaveTargetNewVersion(const std::string& configFile, const uint64_t& version)
{
	FILE* targetFile;
...
bool ZLauncherPatcherThread::SimpleDownloadFile(const std::string& URL, const std::string& targetPath /*= "./"*/)
{
...
bool ZLauncherPatcherThread::SelfUpdate(bool& updateFound)
{
